...
const countryCodes = [
  ...
  { code: "+673", country: "Brunei Darussalam", flag: "🇧🇳" },
...
const CountryCodeSelect: React.FC<CountryCodeSelectProps> = ({ value, onChange, showCountryName = false }) => {
  // Convert country codes to options for react-select
  const options: CountryOption[] = countryCodes.map(({ code, country, flag }) => ({
    value: code,
    label: (
      <div className="flex items-center gap-2">
        ...
        <span>{code}</span>
        {showCountryName && (
          <span className="text-xs text-muted-foreground hidden md:inline">{country}</span>
        )}
      </div>
    ),
    code
  ...
  return (
    <Select
      ...
      onChange={(option) => option && onChange((option as CountryOption).code)}
      options={options}
